...
public class ProductReviewController {
    ...
    public ResponseEntity addReview(@RequestBody @Valid ProductReview review, Errors errors){
        ...
        Integer isReviewd=productReviewService.addReview(review);
        if (isReviewd==1){
            return ResponseEntity.ok().body(new ApiResponse("Your review has been submitted"));
        }
        if (isReviewd==2){
            return ResponseEntity.badRequest().body(new ApiResponse("Product Name not found"));
        }
        return ResponseEntity.badRequest().body(new ApiResponse("ID not found"));
    }

    @PutMapping("/update/{id}")
    public ResponseEntity updateReviews(@PathVariable Integer id, @RequestBody @Valid ProductReview review, Errors errors) {
        if (errors.hasErrors()) {
            String message = errors.getFieldError().getDefaultMessage();
            return ResponseEntity.badRequest().body(message);
        }
    ...
    public ResponseEntity getProductReviews(@PathVariable Integer id){
        List<ProductReview> getReviews=productReviewService.getProductReviews(id);
        if (getReviews!=null){
